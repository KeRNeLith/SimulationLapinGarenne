...
std::vector< std::pair<rabbits_t, rabbits_t> > YoungRabbit::update()
{
    ...
    newAdultsByMonth[4].first = (rabbits_t) 20/100 * m_youngRabbits[m_youngRabbits.size()-1].first;			// TODO A revoir, modifier 20% par une valeur plus adéquate
	newAdultsByMonth[4].second = (rabbits_t)20 / 100 * m_youngRabbits[m_youngRabbits.size() - 1].second;			// TODO A revoir, modifier 20% par une valeur plus adéquate
